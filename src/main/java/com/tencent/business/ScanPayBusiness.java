...
public class ScanPayBusiness {
    ...
    private boolean doOnePayQuery(ScanPayResData scanPayResData, String outTradeNo) throws Exception {
        ...
        this.turnQueryToScan(scanPayResData, scanPayQueryResData);
     ...
     * @throws Exception
     */
    private boolean doOneReverse(String subMchId, String outTradeNo) throws Exception {

        sleep(waitingTimeBeforeReverseServiceInvoked);//等待一定时间再进行查询，避免状态还没来得及被更新

        String reverseResponseString;

        ReverseReqData reverseReqData = new ReverseReqData(subMchId, "", outTradeNo);
     ...
     * @throws InterruptedException
     */
    private void doReverseLoop(String subMchId, String outTradeNo) throws Exception {
        //初始化这个标记
        needRecallReverse = true;
    ...
    private void turnQueryToScan(ScanPayResData payResData, ScanPayQueryResData queryResData) {
        payResData.setAppid(queryResData.getAppid());
        payResData.setAttach(queryResData.getAttach());
        payResData.setBank_type(queryResData.getBank_type());
        payResData.setCoupon_fee(queryResData.getCoupon_fee());
        payResData.setDevice_info(queryResData.getDevice_info());
        payResData.setErr_code(queryResData.getErr_code());
        payResData.setErr_code_des(queryResData.getErr_code_des());
        ...
        payResData.setResult_code(queryResData.getResult_code());
        payResData.setReturn_code(queryResData.getReturn_code());
        payResData.setReturn_msg(queryResData.getReturn_msg());
        payResData.setSign(queryResData.getSign());
        payResData.setTime_end(queryResData.getTime_end());
        payResData.setTotal_fee(queryResData.getTotal_fee());
        payResData.setTrade_type(queryResData.getTrade_type());
        payResData.setTransaction_id(queryResData.getTransaction_id());
    }
}
